...
function cloudworx_setup() {
	
	//Register a theme image size.
	add_theme_support( 'post-thumbnails' );
	add_image_size( 'cloudworx-large-thumb', 830 );
	add_image_size( 'cloudworx-medium-thumb', 550, 400, true );
	add_image_size( 'cloudworx-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'cloudworx' )
	) );
...
add_action( 'after_setup_theme', 'cloudworx_setup' );


/*--- Theme preloader ---*/
function cloudworx_preloader() {
	?>
	<div class="preloader">
	    <div class="spinner">
...
add_action('cloudworx_before_site', 'cloudworx_preloader');
...
function cloudworx_grid_post_meta(){
...
function cloudworx_excerpt_more($more) {
	global $post;
	$layout = theme_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'cloudworx') . '</a>';
}
add_filter('excerpt_more', 'cloudworx_excerpt_more');


/*--- Enqueue scripts and styles ---*/
function cloudworx_scripts() {
	...
	wp_enqueue_script( 'cloudworx-main', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'cloudworx-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	
	/* wp_enqueue_style( 'cloudworx-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true ); */
	
}
add_action( 'wp_enqueue_scripts', 'cloudworx_scripts' );

/**
 * Enqueue Bootstrap
 */
function cloudworx_enqueue_bootstrap() {
	wp_enqueue_style( 'cloudworx-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'cloudworx_enqueue_bootstrap', 9 );



/*--- Register widget area ---*/
add_action( 'widgets_init', 'cloudworx_widgets_init' );
function cloudworx_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'cloudworx' ),
        'id' => 'sidebar-1',
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'cloudworx' ) . $i,
...
function cloudworx_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'cloudworx' ), get_the_title() ),
...
function cloudworx_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'cloudworx').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "cloudworx");
        echo '"<em>';
...
function cloudworx_recommended_plugin() {
...
add_action( 'tgmpa_register', 'cloudworx_recommended_plugin' );

function cloudworx_upsell_script() {
...
add_action( 'customize_controls_enqueue_scripts', 'cloudworx_upsell_script' );
...
add_filter( 'pre_set_site_transient_update_themes', 'cloudworx_check_for_update' );
function cloudworx_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	'after' => 1,// In days
	'interval' => 30,// In days
	'pro_url' => 'https://popularfx.com/themes/wordpress/corporate/Cloudworx_Pro',
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/Cloudworx',
